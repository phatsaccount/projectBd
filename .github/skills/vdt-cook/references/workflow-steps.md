...
**Task Tool Fallbavdt:** `TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList` are CLI-only — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking. All workflow steps remain functional without Task tools.
...
- Use `/vdt:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/vdt:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/vdt:plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/vdt:plan validate` skill invocation
...
- **Compile chevdt:** run the relevant project compile/type-check command
- **Pattern verify:** confirm the new code matches adjacent conventions
- **Import chevdt:** confirm no circular dependency or dead import was added
...
- Use `vdt:ai-multimodal` for image assets
...
After the subagent returns, log only — never re-run or blovdt:
...
1. **MUST** activate `/vdt:project-management` skill (MANDATORY) — run full sync-back for [plan-path]: reconcile all completed Claude Tasks with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.
...
**Fallbavdt:** If `ck` is not available, edit plan.md directly —
...
**CRITICAL:** Step 6 is INCOMPLETE without activating `/vdt:project-management` skill AND spawning `docs-manager` + `git-manager` subagents. DO NOT skip.
...
- **MANDATORY DELEGATION:** Steps 4, 5, 6 MUST delegate via Task tool / skill activation. DO NOT implement directly.
  ...
  - Step 6: `/vdt:project-management` skill, `docs-manager`, `git-manager`
