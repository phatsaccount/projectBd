...
3. If no plan found → Ask user to specify path or run `/vdt:plan` first
...
For each finding, chevdt: does the `Evidence:` field contain at least one `file:line` citation (pattern `path/to/file.ext:NNN`)? If not, auto-set disposition to **Reject** with rationale "No codebase evidence." Do not evaluate merit for evidence-free findings.
...
## Next Steps
Remind user they can run `/vdt:plan validate {plan-directory-path}` before implementation.
When the user approves implementation, run `/vdt:cook {ABSOLUTE_PATH_TO_PLAN_DIR}/plan.md`.
