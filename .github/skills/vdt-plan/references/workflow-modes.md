...
4. **Implementation option:** `/vdt:cook {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `/vdt:scout` to search codebase
...
7. **Context reminder:** `/vdt:cook {absolute-plan-path}/plan.md`
...
2. Read all relevant docs and use `/vdt:scout` across the affected areas
...
8. Output the standard `/vdt:cook {absolute-plan-path}/plan.md` reminder
...
7. **Context reminder:** `/vdt:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `/vdt:plan red-team {plan-directory-path}`.
```
/vdt:plan red-team {plan-directory-path}
```
...
| `auto` | Run `/vdt:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
/vdt:plan validate {plan-directory-path}
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `/vdt:cook {path}/plan.md` |
| hard | `/vdt:cook {path}/plan.md` |
| deep | `/vdt:cook {path}/plan.md` |
| parallel | `/vdt:cook --parallel {path}/plan.md` |
| two | `/vdt:cook {path}/plan.md` |

If planning ran with `--tdd`, append `--tdd` to the reminder above so cook keeps
the tests-first execution path. Example:
`/vdt:cook {path}/plan.md --tdd`
